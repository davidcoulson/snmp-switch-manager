...
class SwitchSnmpClient:
    ...
    async def _walk_ipv4(self) -> None:
        ...
        # 2) ipCidrRoutePrefixLength + ipCidrRouteIfIndex -> pick most specific prefix per ifIndex
        ...
        # Store only if we actually found anything; otherwise leave cache unchanged
        if ip_to_index:
            self.cache["ipIndex"] = ip_to_index
        if ip_to_mask:
            self.cache["ipMask"] = ip_to_mask
